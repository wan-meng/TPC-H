...
def plotmonetvspg():
    ...
    p1=py.bar(x1,y1,width=0.35,align='center')
    p2=py.bar(x1+0.4,y2,width=0.4,color='green',align='center')
    py.xlabel('queries')
    py.xlim(0,22)
    py.ylabel('reponse time in seconds')
    #py.xticks((p1,p2),('m','p'))
    py.legend((p1,p2),('monetdb','postgresql'))
    py.title('TPC-H benchmark with Postgresql and MonetDB')
    py.savefig('monetvspg.jpg')
